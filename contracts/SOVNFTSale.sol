...
import "./Interfaces/ISOVRewards.sol";

contract SOVNFTSale is
    Initializable,
    ERC721Upgradeable,
    ...
    ERC2981Upgradeable
{
    ...
    function initialize(
        string memory _name,
        string memory _symbol, //maxpresalelimit , mintoken, recieveradr, royaltyAmt
    ...
    function validateNftLimit(address _addr, uint256 nftQty) internal view {
        ...
        uint256 count = tokenBalance >= tierMap[1].minValue && tokenBalance <= tierMap[1].maxValue
            ? 1
            : tokenBalance >= tierMap[2].minValue && tokenBalance <= tierMap[2].maxValue
            ? 2
            : tokenBalance >= tierMap[3].minValue && tokenBalance <= tierMap[3].maxValue
            ? 3
            : 4;
    ...
    function setRewardContract(ISOVRewards _sovRewards) external onlyOwner {
        sovRewards = _sovRewards;
    }
    ...
    function buyPresale(uint256 nftQty) external payable {
        ...
        require(
            msg.value == nftQty * (SaleDetailMap[saleId].mintCost + SaleDetailMap[saleId].cost),
    ...
    function minting(CATEGORY[] memory _category, string[] memory _tokenURI) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if ((rewardEnable && from != address(0)) && checkTokenRewardEligibility(tokenId)) {
            uint256 _rewards = sovRewards.getRewardsCalc(tokenId);
            sovRewards.updateRewardAmount(from, _rewards);
        }
